...
def test_known_experiment_with_unknown_treatment_is_inert() -> None:
    with _topology() as (_, telemetry, endpoints):
        payload, _, trace_id = _request_with_allocations(
            telemetry,
            endpoints["orchestrator-a"],
            "Hello",
            {SPECIALIST_EXPERIMENT: "not-registered"},
        )
        assert payload["result"] == legacy_specialist_answer("Hello")
        ...
        assert specialist["attributes"]["specialist.mode"] == "control"
